...
bool KmlGenerator::addKmlPointBody(std::string data_name, std::string data_str, int visibility)
{
  ...
  body_ += "<open>1</open>\n"
           ...
           "\t\t\t<text><![CDATA[<b><font color=\"#CC0000\" "
           "size=\"+3\">$[name]</font></b><br>$[description]</font><br/>]]></text>\n"
           "\t\t</BalloonStyle>\n"
           "\t</Style>\n"
           "<Folder id=\"ID2\">\n"
           "\t<name>" +
                                         ...
                                         std::vector<kml_utils::OtherInfo> other_info_vector)
{
  ...
  if (update_flag && llh[0] != 0 && llh[1] != 0)
  {
    if (kml_type_ == KMLType::LINE)
    {
      ...
      data_ss << std::setprecision(std::numeric_limits<double>::max_digits10) << llh[1] << ","
              << std::setprecision(std::numeric_limits<double>::max_digits10) << llh[0] << ","
              << std::setprecision(std::numeric_limits<double>::max_digits10) << llh[2] << std::endl;
      str = data_ss.str();
    }
    else if (kml_type_ == KMLType::POINT)
    {
                                              ...
                                              std::vector<kml_utils::OtherInfo> other_info_vector)
{
  ...
  data = "\t<Placemark>\n\
         ...
         std::to_string(sequence) + "</TD></TR>\n" +
         "\t\t\t\t\t\t\t\t<TR ALIGN=RIGHT><TD ALIGN=LEFT>Timestamp: </TD><TD>" + kml_utils::makeDouble2String(time) +
         "</TD></TR>\n\
  ...
  if (!other_info_vector.empty())
  {
    ...
    {
      data += "\t\t\t\t<TR ALIGN=RIGHT><TD ALIGN=LEFT>" + oi.name + ": </TD><TD>" + oi.value_str + "</TD></TR>\n";
    }
  }
  data += "\t\t\t</TABLE>\n"
          "]]></description>\n"
          ...
          +"</styleUrl>\n"
           "\t\t<Point>\n"
           "t\t\t<coordinates>" +
          kml_utils::makeDouble2String(llh[1]) + "," + kml_utils::makeDouble2String(llh[0]) + "," +
...
bool KmlGenerator::addPointVector2LineKML(const std::vector<kml_utils::Point>& point_vector, std::string data_name,
                                          int visibility, ColorType ct)
{
  kml_type_ = KMLType::LINE;
  color_type_ = ct;
...
bool KmlGenerator::addLinesKML(const kml_utils::Lines& lines)
{
  ...
  if (!addAllLineStyles())
    return false;
  if (!addKmlFolderBegin(lines.header))
    return false;

  for (const auto& line : lines.lines)
  {
    if (!addKmlLineBody(line))
      return false;
  }
...
bool KmlGenerator::addPointVector2PointKML(const std::vector<kml_utils::Point>& point_vector, std::string data_name,
                                           int visibility, ColorType ct)
{
  kml_type_ = KMLType::POINT;
  color_type_ = ct;
